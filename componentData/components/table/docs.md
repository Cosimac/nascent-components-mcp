展示大量结构化数据，方便用户阅读、查找、分析（比较数据）。
## 设计师专属
安装 [Kitchen Sketch 插件 💎](https://kitchen.alipay.com/)，两步就可以自动生成 NUI V3 表格组件。
## 何时使用
- 当有大量结构化的数据需要展现时；
...
```
## API
另外我们封装了 `ProTable`，在 `NUI V3` Table 之上扩展了更多便捷易用的功能，内置搜索、筛选、刷新等常用表格行为，并为多种类型数据展示提供了内置格式化，欢迎尝试使用。
### Table
| 参数 | 说明 | 类型 | 默认值 | 版本 |
| --- | --- | --- | --- | --- |
...
| columnWidth | 自定义列表选择框宽度 | string \| number | `48px` |  |
...
| x | 设置横向滚动，也可用于指定滚动区域的宽，可以设置为像素值，百分比，true 和 ['max-content'](https://developer.mozilla.org/zh-CN/docs/Web/CSS/width#max-content)。注意：设置为 true 时，可能会导致配置的列宽不生效，可改为'100%'。 | string \| number \| true | - |
...
## 在 TypeScript 中使用
```tsx
import { Table } from '@nascent/nui';
import { ColumnsType } from '@nascent/nui/es/table';
interface User {
  key: number;
  name: string;
}
